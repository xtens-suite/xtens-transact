...
KnexStrategy.prototype = {
    ...
    putMetadataValuesIntoEAV: function(data, eavValueTableMap) {
        ...
        return knex.transaction(function(trx) {
            ...
            return knex('data_type').where({id: data.type}).first('classTemplate').transacting(trx)

            .then(function(row) {
                
                // identify the table (e.g. data, subject, sample...)
                var table = row.classTemplate.toLowerCase().toUnderscore();
                console.log("KnexStrategy.putMetadataValuesIntoEAV - entity table is: " + table);
                ...
                return BluebirdPromise.map(Object.keys(data.metadata), function(metadataField) {
                    ...
                    .then(function(eavAttribute) {
                        ...
                        if (data.metadata[metadataField].value) {
                            ...
                            if (eavAttribute.hasUnit && data.metadata[metadataField].unit) {
                                eavValue.unit = data.metadata[metadataField].unit;
                            }

                            return knex.returning('id').insert(eavValue).into(eavValueTableMap[eavAttribute.fieldType]).transacting(trx);
                        }
                        ...
                        // otherwise it is a loop!!
                            ...
                            var eavValues = data.metadata[metadataField].values.map(function(value, index) {
                                var unit = data.metadata[metadataField].units[index] || undefined;
                                return {
                                    'entityTable': table,
                                    'entityId': data.id,
                                    'attribute': eavAttribute.id,
                            ...
                            return knex.returning('id').insert(eavValues).into(eavValueTableMap[eavAttribute.fieldType]).transacting(trx);
                        }

                        // something is wrong, throw new error  
                        else {
                            throw new Error("metadata field missing or it does not possess a valid value");
                        }
                    });
                ...
                .then(function(ids) {
                    console.log("KnexStrategy.putMetadataValuesIntoEAV - inserted successfully new metadata value: IDS " + ids + 
                                "; value: " + data.metadata[metadataField] );
                    return ids;
                }); 
            })
