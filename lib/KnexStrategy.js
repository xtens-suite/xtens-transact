            ...
            .then(function(row) {
                ...
                return BluebirdPromise.map(Object.keys(data.metadata), function(metadataField) {
                    ...
                    .then(function(eavAttribute) {
                        if (eavAttribute.length > 0) {
                            throw new Error("more than one attribute was restrieved!!");
                        }
                        ...
                        if (data.metadata[metadataField].value) {
                            ...
                            if (eavAttribute.has_unit && data.metadata[metadataField].unit) {
                        ...
                        // otherwise it is a loop!!
                            ...
                            var eavValues = data.metadata[metadataField].values.map(function(value, index) {
                                var unit = (data.metadata[metadataField].units && data.metadata[metadataField].units[index]) || undefined;
                                return {
                                    // 'entity_table': table,
                                    'entity': data.id,
                                    'attribute': eavAttribute.id,
